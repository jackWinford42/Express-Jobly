...
describe("GET /users/:username", function () {
  ...
  test("auth and works for the same user", async function () {
    const resp = await request(app)
        .get(`/users/u1`)
        .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
...
describe("POST /users/:username/jobs/:id", function () {
  test("works with valid username and id", async function () {
    const resp = await request(app)
  ...
  test("does not work with invalid username and valid id", async function () {
    const resp = await request(app)
      .post(`/users/invalid/jobs/${jobIds[0]}`)
      .set("authorization", `Bearer ${u4Token}`);
    expect(resp.statusCode).toEqual(404)
  })
  test("does not work with valid username and invalid id", async function () {
    const resp = await request(app)
      .post(`/users/u1/jobs/2348098`)
      .set("authorization", `Bearer ${u4Token}`);
    expect(resp.statusCode).toEqual(404)
  })
  test("unauthorized user cannot apply", async function () {
    const resp = await request(app)
